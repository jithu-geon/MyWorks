...
var fs = require("fs");
function affected(command, parsedArgs, args) {
    var apps;
    var e2eProjects;
    var libs;
    var projects;
    var buildables;
    var rest;
    var workspaceResults = new workspace_results_1.WorkspaceResults(command);
    try {
        if (parsedArgs.all) {
            rest = args;
            ...
            e2eProjects = shared_1.getAllE2ENames()
                .filter(function (app) { return !parsedArgs.exclude.includes(app); })
                .filter(function (project) {
                return !parsedArgs.onlyFailed || !workspaceResults.getResult(project);
            });
            ...
            buildables = shared_1.getAllBuildables()
                .filter(function (project) { return !parsedArgs.exclude.includes(project); })
                .filter(function (project) {
                return !parsedArgs.onlyFailed || !workspaceResults.getResult(project);
            });
            ...
            e2eProjects = shared_1.getAffectedE2e(p.files)
                .filter(function (project) { return !parsedArgs.exclude.includes(project); })
                .filter(function (project) {
                return !parsedArgs.onlyFailed || !workspaceResults.getResult(project);
            });
            ...
            buildables = shared_1.getAffectedBuildables(p.files)
                .filter(function (project) { return !parsedArgs.exclude.includes(project); })
                .filter(function (project) {
                return !parsedArgs.onlyFailed || !workspaceResults.getResult(project);
            });
    ...
    switch (command) {
        ...
        case 'e2e':
            ...
            break;
        case 'dep-graph':
            dep_graph_1.generateGraph(yargsParser(rest), projects);
            break;
        case 'lib':
...
function build(projects, parsedArgs, workspaceResults) {
    if (projects.length > 0) {
        var normalizedArgs = filterNxSpecificArgs(parsedArgs);
        var message = "Building " + projects.join(', ');
        if (normalizedArgs.length > 0) {
            message += " with flags: " + normalizedArgs.join(' ');
        }
        console.log(message);
        runCommand('build', projects, parsedArgs, normalizedArgs, workspaceResults, 'Building ', 'Build succeeded.', 'Build failed.');
    }
    else {
        console.log('No projects to build');
    }
}
function test(projects, parsedArgs, workspaceResults) {
    var depGraph = shared_1.readDepGraph();
    var sortedProjects = topologicallySortProjects(depGraph);
    var sortedAffectedProjects = sortedProjects.filter(function (pp) { return projects.indexOf(pp) > -1; });
    var projectsToTest = sortedAffectedProjects.filter(function (p) {
        var matchingProject = depGraph.projects.find(function (pp) { return pp.name === p; });
        return (matchingProject.type !== affected_apps_1.ProjectType.e2e &&
            !!matchingProject.architect['test']);
    });
    if (projectsToTest.length > 0) {
        var normalizedArgs = ['--no-watch'].concat(filterNxSpecificArgs(parsedArgs));
        var message = "Testing " + projectsToTest.join(', ');
        if (normalizedArgs.length > 0) {
            message += " with flags: " + normalizedArgs.join(' ');
        }
        console.log(message);
        runCommand('test', projectsToTest, parsedArgs, normalizedArgs, workspaceResults, 'Testing ', 'Tests passed.', 'Tests failed.');
    }
    else {
        console.log('No projects to test');
    }
}
function lint(projects, parsedArgs, workspaceResults) {
    var depGraph = shared_1.readDepGraph();
    var sortedProjects = topologicallySortProjects(depGraph);
    var sortedAffectedProjects = sortedProjects.filter(function (pp) { return projects.indexOf(pp) > -1; });
    var projectsToLint = sortedAffectedProjects.filter(function (p) {
        var matchingProject = depGraph.projects.find(function (pp) { return pp.name === p; });
        return !!matchingProject.architect['lint'];
    });
    if (projectsToLint.length > 0) {
        var normalizedArgs = filterNxSpecificArgs(parsedArgs);
        var message = "Linting " + projectsToLint.join(', ');
        if (normalizedArgs.length > 0) {
            message += " with flags: " + normalizedArgs.join(' ');
        }
        console.log(message);
        runCommand('lint', projectsToLint, parsedArgs, normalizedArgs, workspaceResults, 'Linting ', 'Linting passed.', 'Linting failed.');
    }
    else {
...
function runCommand(command, projects, parsedArgs, args, workspaceResults, iterationMessage, successMessage, errorMessage) {
    return __awaiter(this, void 0, void 0, function () {
        var packageJson, e_1, failedProjects_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, runAll(projects.map(function (app) { return "ng " + command + " -- " + args.join(' ') + " --project=" + app; }), {
                ...
                case 5:
                    ...
                    projects.forEach(function (project) {
                        ...
                        try {
                            child_process_1.execSync("node " + ngPath() + " " + command + " " + args.join(' ') + " --project=" + project, {
                                stdio: [0, 1, 2]
                            });
                            workspaceResults.success(project);
                        }
...
function e2e(apps, parsedArgs, workspaceResults) {
    if (apps.length > 0) {
        var args_1 = filterNxSpecificArgs(parsedArgs);
        console.log("Testing " + apps.join(', '));
        apps.forEach(function (app) {
            try {
                child_process_1.execSync("node " + ngPath() + " e2e " + args_1.join(' ') + " --project=" + app, {
                    stdio: [0, 1, 2]
                });
        ...
        workspaceResults.saveResults();
        workspaceResults.printResults(parsedArgs.onlyFailed, 'E2E Tests passed.', 'E2E Tests failed.');
        if (workspaceResults.hasFailure) {
            process.exit(1);
        }
    }
